...
<Panel id="monitors" title="自定义监控器" {count} {loading} {error}>
	<div class="monitors-content">
		{#if monitors.length === 0 && !loading && !error}
			<div class="empty-state">
				<p>未配置任何监控器</p>
				{#if onCreateMonitor}
					<button class="create-btn" onclick={onCreateMonitor}> + 创建监控器 </button>
				{/if}
			</div>
		{:else}
			<div class="monitors-header">
				<span class="active-count">{activeMonitors.length} 活跃</span>
				{#if onCreateMonitor}
