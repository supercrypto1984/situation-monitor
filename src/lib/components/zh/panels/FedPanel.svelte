<script lang="ts">
    ...
    function getTranslatedIndicatorName(englishName: string): string {
        switch (englishName) {
            case 'Federal Funds Rate': return '联邦基金利率';
            case 'CPI (MoM)': return 'CPI (月环比)';
            case '10-Year Treasury Yield': return '10年期国债收益率';
    ...
    function getTranslatedTypeLabel(typeLabel: string): string {
        switch (typeLabel.toLowerCase()) {
            case 'powell': return '鲍威尔';
            case 'monetary': return '货币政策';
            case 'speech': return '演讲';
            case 'testimony': return '证词';
            case 'announcement': return '声明';
...
<Panel id="fed" title="美联储" count={newsState.items.length} {loading} {error}>
			...
			<div class="indicator-cards">
				{#each indicatorList as indicator (indicator.seriesId)}
					<div class="indicator-card">
						<div class="indicator-label">{getTranslatedIndicatorName(indicator.name)}</div>
	...
	{:else if !hasApiKey && !loading}
		<div class="no-api-key">
			<span class="no-api-key-text">添加 VITE_FRED_API_KEY 以查看经济指标</span>
		</div>
	{/if}
					...
					<div class="fed-news-item" class:powell={item.isPowellRelated}>
						<div class="fed-news-header">
							<div class="fed-news-badges">
								<Badge text={getTranslatedTypeLabel(item.typeLabel)} variant={getTypeVariant(item.type)} />
