<script lang="ts">
	...
	function getTranslatedStatus(status: string): string {
		switch (status) {
			case 'viral': return '病毒式传播';
			case 'spreading': return '正在传播';
			case 'emerging': return '新兴';
			case 'crossing': return '跨越';
	...
	function getTranslatedSeverity(severity: string): string {
		switch (severity) {
			case 'high': return '高';
...
<Panel id="narrative" title="叙事追踪器" {loading} {error}>
	{#if news.length === 0 && !loading && !error}
		<div class="empty-state">数据不足以进行叙事分析</div>
	{:else if analysis}
		<div class="narrative-content">
					...
					{#each analysis.emergingFringe.slice(0, 4) as narrative}
						<div class="narrative-item">
							<div class="narrative-header">
								...
								<Badge
									text={getTranslatedStatus(narrative.status).toUpperCase()}
			...
			{#if analysis.fringeToMainstream.length > 0}
				<div class="section">
					<div class="section-title">边缘 → 主流跨越</div>
					{#each analysis.fringeToMainstream.slice(0, 3) as crossover}
						<div class="crossover-item">
							...
							<div class="crossover-level">
								跨越级别: {Math.round(crossover.crossoverLevel * 100)}%
							</div>
						</div>
					{/each}
				</div>
					...
					{#each analysis.disinfoSignals.slice(0, 3) as signal}
						<div class="disinfo-item">
							<div class="disinfo-header">
								...
								<Badge
									text={getTranslatedSeverity(signal.severity).toUpperCase()}
