<script lang="ts">
	...
	import {
		...
		THREAT_COLORS,
		WEATHER_CODES
	} from '$lib/config/zh/map'; // Import from Chinese config
	...
	async function showEnhancedTooltip(
		...
		const lines = [`🕐 本地时间: ${localTime}`]; // Translated
		...
		if (weather && tooltipVisible) {
			tooltipContent = {
				...
				color,
				lines: [
					`🕐 本地时间: ${localTime}`, // Translated
					`${weather.condition} ${weather.temp}°F, ${weather.wind} 英里/小时` // Translated
				]
			};
		}
	}
	...
	async function initMap(): Promise<void> {
		...
		try {
			...
			OCEANS.forEach((o) => {
				...
				if (x && y) {
					mapGroup
						...
						.attr('opacity', 0.6)
						.text(o.name); // Ocean names are not translated, as they are proper nouns/codes
				}
			});
			...
			function getTranslatedDescription(key: string, originalDesc: string): string {
				switch (key) {
					case 'chokepoint':
						return `⚓ ${originalDesc}`;
					case 'cable landing':
			...
			CHOKEPOINTS.forEach((cp) => {
				...
				if (x && y) {
					...
					mapGroup
						...
						.attr('font-size', '7px')
						.attr('font-family', 'monospace')
						.text(cp.name); // Names are not translated, they are codes/proper nouns
					mapGroup
						.append('circle')
						.attr('cx', x)
						.attr('cy', y)
						...
						.on('mouseenter', (event: MouseEvent) => showTooltip(event, getTranslatedDescription('chokepoint', cp.desc), '#00aaff')) // Translated
			...
			CABLE_LANDINGS.forEach((cl) => {
				...
				if (x && y) {
					...
					mapGroup
						...
						.attr('fill', 'transparent')
						.attr('class', 'hotspot-hit')
						.on('mouseenter', (event: MouseEvent) => showTooltip(event, getTranslatedDescription('cable landing', cl.desc), '#aa44ff')) // Translated
						.on('mousemove', moveTooltip)
						.on('mouseleave', hideTooltip);
				}
			});
			...
			NUCLEAR_SITES.forEach((ns) => {
				...
				if (x && y) {
					...
					mapGroup
						...
						.attr('fill', 'transparent')
						.attr('class', 'hotspot-hit')
						.on('mouseenter', (event: MouseEvent) => showTooltip(event, getTranslatedDescription('nuclear site', ns.desc), '#ffff00')) // Translated
						.on('mousemove', moveTooltip)
						.on('mouseleave', hideTooltip);
				}
			});
			...
			MILITARY_BASES.forEach((mb) => {
				...
				if (x && y) {
					...
					mapGroup
						...
						.attr('fill', 'transparent')
						.attr('class', 'hotspot-hit')
						.on('mouseenter', (event: MouseEvent) => showTooltip(event, getTranslatedDescription('military base', mb.desc), '#ff00ff')) // Translated
						.on('mousemove', moveTooltip)
						.on('mouseleave', hideTooltip);
				}
			});
			...
			.forEach((m) => {
				...
				if (x && y) {
					...
					mapGroup
						...
						.attr('fill', 'transparent')
						.on('mouseenter', (event: MouseEvent) =>
							showTooltip(event, getTranslatedDescription('monitor', m.name), color, [ // Translated
								m.location?.name || '',
								m.keywords.join(', ')
							])
						)
