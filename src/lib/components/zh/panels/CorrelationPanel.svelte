<script lang="ts">
	...
	function getTranslatedLevel(level: string): string {
		switch (level) {
			case 'high': return '高';
			case 'elevated': return '升高';
			case 'surging': return '激增';
...
<Panel id="correlation" title="模式分析" {loading} {error}>
	{#if news.length === 0 && !loading && !error}
		<div class="empty-state">数据不足以进行分析</div>
	{:else if analysis}
		<div class="correlation-content">
					...
					{#each analysis.emergingPatterns.slice(0, 3) as pattern}
						<div class="pattern-item">
							<div class="pattern-header">
								...
								<Badge
									text={getTranslatedLevel(pattern.level).toUpperCase()}
			...
			{#if analysis.crossSourceCorrelations.length > 0}
				<div class="section">
					<div class="section-title">跨源链接</div>
